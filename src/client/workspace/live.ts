...
export function mountLive(controller: LiveWorkspaceController, signal: AbortSignal, updatePreview: () => void) {
  ...
  function render() {
    ...
    text('live-status', s.error ? `${s.error}${s.bootstrap ? ' Displayed records are the last verified snapshot.' : ''}` : (s.loading ? 'Fetching and verifying reference, state and history...'
    ...
    for (const [id, enabled] of [['live-confirm', s.canConfirm], ['live-run', s.canRun], ['live-accept', s.canAccept], ['live-download', s.canDownload],
    ...
    text('live-accept-gate', s.canAccept ? `Accept ${b?.design?.selectedCandidateRevisionId} with its verified current checks.`
      : 'Acceptance requires inspection of the server-selected live reviewable revision with complete current checks.');
    ...
    text('live-check-count', r ? `${candidate.checks.filter(check => check.state === 'passed').length}/${r.requiredChecks.length} passed` : 'No checks');
    ...
    const key = pretty([s.loading, s.trusted, s.viewedRevisionId, comparison.value, candidate?.status, candidate?.checkBundleHash,
      candidate?.artifacts, s.reference]);
    if (key !== previewKey) { previewKey = key; if (active) updatePreview(); }
  }
  ...
  for (const [id, action] of [['live-confirm', controller.confirmRequirements], ['live-run', controller.requestRun], ['live-accept', controller.acceptRevision],
    ['live-refresh', controller.refresh], ['live-reconnect', controller.reconnect]] as const) {
    element(id).addEventListener('click', () => { void action(); }, { signal });
  }
  ...
  signal.addEventListener('abort', () => { unsubscribe(); if (poll) clearTimeout(poll); }, { once: true });
  render();
  return {
    open() { active = true; if (!initialized) { initialized = true; void controller.load(); } else { void controller.reconnect(); } schedulePoll(); },
    close() { active = false; if (poll) clearTimeout(poll); },
