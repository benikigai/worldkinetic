...
- `WORLDKINETICS_PUBLIC_RUN_LIMIT`: default `3`; explicitly set `unlimited` for the authorized password-gated demo period to remove both visitor session/shared run ceilings and the visitor design-count ceiling. Null status fields represent unlimited, never a fabricated large remaining count. Password checks, eight-hour sessions, login throttles, execution lock, timeout and verification remain in force. Returning to bounded mode requires an explicit deployment with verified carried usage.
